		...
		if (tif->tif_dirlistoff[n] == diroff) {
			if (tif->tif_dirlistdirn[n] == dirn) {
				...
				TIFFWarningExt(tif->tif_clientdata, "_TIFFCheckDirNumberAndOffset",
					"TIFF directory %"PRId32" has IFD looping to directory %"PRIu16" at offset 0x%"PRIx64" (%"PRIu64")",
					(int32_t)dirn-1, tif->tif_dirlistdirn[n], diroff, diroff);
				return 0;
			}
		}
	}
...
TIFFFetchNormalTag(TIFF* tif, TIFFDirEntry* dp, int recover)
{
	...
	switch (fip->set_field_type)
	{
		case TIFF_SETGET_UNDEFINED:
				TIFFErrorExt(tif->tif_clientdata, "TIFFFetchNormalTag",
					"Defined set_field_type of custom tag %"PRIu32" (%s) is TIFF_SETGET_UNDEFINED and thus tag is not read from file",
