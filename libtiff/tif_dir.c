...
TIFFAdvanceDirectory(TIFF* tif, uint64_t* nextdiroff, uint64_t* off, uint16_t* nextdirnum)
{
	...
	if (*nextdiroff != 0) {
		...
		if (!_TIFFCheckDirNumberAndOffset(tif, *nextdirnum, *nextdiroff)) {
			TIFFWarningExt(tif->tif_clientdata, module, "the next directory %"PRIu16" at offset 0x%"PRIx64" (%"PRIu64") might be an IFD loop. Treating directory %"PRId32" as last directory",
				*nextdirnum, *nextdiroff, *nextdiroff, (int32_t)(*nextdirnum) - 1);
			*nextdiroff = 0;
			(*nextdirnum)--;
		}
	}
